...
func (m *multilineStage) Run(in chan Entry) chan Entry {
	...
	go func() {
		...
		// the hot path (every incoming entry) can update the timer in O(1)
		// without scanning all streams. It is reset to zero and rescanned
		// after a flush, which is rare (once per completed block).
		...
		// after a flush removes a stream from contention. Includes streams with
		// currentLines==0 (flushed at max_lines) so they are cleaned up by the
		// timer even when idle. Called infrequently — once per flushed block.
		rescanDeadline := func() {
		...
		for {
			...
			case e, ok := <-in:
				...
				// O(1) hot path: the entry just updated lastSeen for its stream.
			...
			case <-timer.C:
				...
				// (flushed at max_lines and then gone idle) are deleted silently.
				now := time.Now()
...
// accumulated — mirroring the goroutine behaviour of the original Run.
func (m *multilineStage) processEntry(e Entry) []Entry {
	...
	// Lazy max_wait_time: flush stale block before processing new entry.
	if hasState && state.currentLines > 0 && time.Since(state.lastSeen) >= m.cfg.MaxWaitTime {
		level.Debug(m.logger).Log("msg", fmt.Sprintf("flush multiline block due to %v timeout", m.cfg.MaxWaitTime), "block", state.buffer.String())
		out = append(out, m.flushState(state))
	}

	// Evaluate the regex once; the result is used both for the pre-stream
	// pass-through check and for the start-line flush/update below.
...
func (m *multilineStage) flushState(s *multilineState) Entry {
	if s.buffer.Len() == 0 {
		panic("multilineStage: flushState called with empty buffer; currentLines and buffer are out of sync")
	}

...
// preserved correctly across two consecutive max_lines flushes on the same
// stream: post-flush continuations should carry the timestamp of the most
// recent start line, not the current entry.
func TestMultilineStageMultipleMaxLinesFlushes(t *testing.T) {
	...
	entry := func(line string, offset time.Duration) Entry {
		return Entry{
			Extracted: map[string]any{},
			Entry:     loki.NewEntry(lset.Clone(), push.Entry{Timestamp: startTs.Add(offset), Line: line}),
		}
	}

	out := processEntries(stage,
		entry("START line 1", 0),
		entry("continuation 1", time.Second),   // MaxLines=2 reached → flush block 1
		entry("continuation 2", 2*time.Second), // accumulated into block 2 (no new start)
		entry("continuation 3", 3*time.Second), // MaxLines=2 reached → flush block 2
		entry("continuation 4", 4*time.Second), // accumulated into block 3 on close
	)

	require.Len(t, out, 3)

	// Block 1: two lines, timestamp from START line 1.
	require.Equal(t, "START line 1\ncontinuation 1", out[0].Line)
	require.Equal(t, startTs, out[0].Timestamp)

	// Block 2: two lines, timestamp preserved from START line 1 (no new start seen).
	require.Equal(t, "continuation 2\ncontinuation 3", out[1].Line)
	require.Equal(t, startTs, out[1].Timestamp)

	// Block 3: one line on close, timestamp preserved from START line 1.
...
// TestFlushStateEmptyBufferPanics verifies that flushState panics when called
// with an empty buffer. The invariant currentLines > 0 ⇒ buffer.Len() > 0
// should always hold; a violation indicates a bug in the caller, not a
// runtime condition, so a panic surfaces it immediately.
func TestFlushStateEmptyBufferPanics(t *testing.T) {
	mcfg := MultilineConfig{Expression: "^START", MaxWaitTime: 3 * time.Second, TrimNewlines: true}
	regex, err := validateMultilineConfig(mcfg)
	require.NoError(t, err)
	stage := &multilineStage{
	...
	require.Panics(t, func() {
		stage.flushState(&multilineState{}) // empty buffer
	})
}

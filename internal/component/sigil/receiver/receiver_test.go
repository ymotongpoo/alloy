...
func TestHandler(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			h := newHandler(log.NewNopLogger(), m, receivers)
			...
			if tc.expectForwards > 0 && tc.receivers > 0 {
				...
				if tc.auth != "" {
					require.Equal(t, tc.auth, mocks[0].lastReq.Headers["Authorization"])
				}
			}
		})
	}
...
func TestHandler_PartialSuccess(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			m := newMetrics(prometheus.NewRegistry())
			h := newHandler(log.NewNopLogger(), m, []sigil.GenerationsReceiver{mock1, mock2})
